...
- `src/pages/[slug]/index.astro` — App landing pages via `getStaticPaths`. Sets `--app-accent` CSS variable per app. Structure: hero → direct-answer block from first FAQ → compact capabilities block from feature data → premium screenshot gallery → FAQ accordion → download CTA → cross-promo placeholders. Includes `SoftwareApplication`, `BreadcrumbList`, `WebPage`, and `FAQPage` JSON-LD structured data.
...
- **Organization schema:** On every page via `BaseLayout.astro` (JSON-LD), including `contactPoint`. `sameAs` array is empty — fill when social accounts go live.
...
- **App screenshots** use a premium staged gallery when real screenshots exist, and a simpler placeholder gallery only when `app.screenshots` is empty.
