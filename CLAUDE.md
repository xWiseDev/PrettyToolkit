...
**Currently, nine apps are live on the App Store: Luxira (QR & Barcode Scanner), Sincefy (countdown widget), Sopora (sleep journal), logfd (food & intake diary), BPIVY (blood-pressure log), Raheva (period tracker), Postivo (intermittent-fasting timer, live since 2026-08-18), Esynit (PDF signing, live since 2026-08-21), and Balanza (BMI & weight tracker, live since 2026-09-01). Skanvo (document/PDF scanner), Swipeve (photo storage cleaner, added 2026-09-07 by owner ruling), Delayo (quit-smoking delay timer, added 2026-09-08 by owner ruling) and Kantevo (snag list / punch list site audit, added 2026-09-14 by owner ruling) are public coming-soon apps.** The homepage spotlights Sincefy and shows eight of the live apps as `AppCard`s plus 1 `PlaceholderCard` hinting at "more coming soon" (Balanza is live but still not in the hardcoded grid - owner call, deferred). Future apps beyond these nine remain confidential — names, categories, and details must NOT appear on the site until they ship.
...
Currently Sincefy, Luxira, Sopora, logfd, BPIVY, Raheva, Postivo, Esynit, and Balanza are in the `apps` array with `status: "live"` and App Store URLs; Skanvo, Swipeve, Delayo and Kantevo are in the array with `status: "coming-soon"` and no App Store URL yet (Swipeve, Delayo and Kantevo have full `privacy` fields because their App Store submissions need the live URL). When the next app ships, add it (including a `privacy` field) and it automatically gets a landing page, per-app privacy policy, footer icon, and cross-promo presence — the homepage spotlight and grid are hardcoded in `index.astro`, so decide whether the new app becomes the spotlight or joins the grid. Never include subscription pricing, trial length, or freemium limits on the website.
...
- `public/icons/` — App icons (512x512 PNG). Currently contains `luxira.png`, `sincefy.png`, `sopora.png`, `logfd.png`, `bpivy.png`, `raheva.png`, `postivo.png`, and `skanvo.png`.
