...
- `src/pages/[slug]/index.astro` — App landing pages via `getStaticPaths`. Sets `--app-accent` CSS variable per app. Structure: hero → direct-answer block from first FAQ → capabilities block from feature data → 6 feature sections (alternating layout) → screenshot gallery → FAQ accordion → download CTA → cross-promo placeholders. Includes `SoftwareApplication`, `BreadcrumbList`, `WebPage`, and `FAQPage` JSON-LD structured data.
