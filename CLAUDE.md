...
Currently Sincefy, Luxira, Sopora, logfd, BPIVY, Raheva, Postivo, Esynit, and Balanza are in the `apps` array with `status: "live"` and App Store URLs; Skanvo and Swipeve are in the array with `status: "coming-soon"` and no App Store URL yet (Swipeve has a full `privacy` field because its App Store submission needs the live URL). When the next app ships, add it (including a `privacy` field) and it automatically gets a landing page, per-app privacy policy, footer icon, and cross-promo presence — the homepage spotlight and grid are hardcoded in `index.astro`, so decide whether the new app becomes the spotlight or joins the grid. Never include subscription pricing, trial length, or freemium limits on the website.
